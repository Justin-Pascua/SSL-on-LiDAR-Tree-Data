...
class TreeClassifier(nn.Module):
  ...
  def fit(self, train_dataloader, val_dataloader, loss_fn, optimizer, epochs, verbose = 1):
    ...
    params:
      ...
      verbose: integer which controls the level of output during training.
                If verbose == 0, then this method only prints when training is done
                If verbose == 1, then this method prints once every epoch
    """
    train_acc_history = []
    train_loss_history = []
    val_acc_history = []
    ...
    for t in range(epochs):
      ...
      train_acc, train_loss, val_acc, val_loss = self._training_eval(train_dataloader,
                                                                     val_dataloader,
                                                                     loss_fn,
                                                                     verbose)
